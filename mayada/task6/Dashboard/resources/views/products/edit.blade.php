...
@section('content')
    ...
    <form action="{{ route('dashboard.products.update', ['id' => $product->id]) }}" method="post"
                ...
                <div class="form-group">
                    ...
                    <select name="status" id="status" class="form-control">
                        @foreach ($statuses as $key => $status)
                            <option @selected($product->status == $key) value="{{ $key }}">{{ $status }}</option>
                        @endforeach
                    </select>
                    @error('status')
                        <div class="alert alert-danger"> {{ $message }}</div>
                ...
                <div class="form-group">
                    ...
                    <select name="barnd_id" id="brand_id" class="form-control">
                        ...
                        @foreach ($brands as $brand)
                            <option @selected($product->barnd_id == $brand->id) value="{{ $brand->id }}">{{ $brand->name_ar }}
            ...
            <div class="col-md-3">
                <div class="form-group">
                    <label for="image">upload image
                        <img src="{{ asset('assets/images/products/' . $product->image) }}" id='img'
                            style="cursor:pointer" alt="photo" class="img-fluid"></label>
        ...
        <button class="btn btn-primary" name="redirect" value="all">edit product</button>
        <button class="btn btn-primary" name="redirect" value="back">edit product & return</button>
    </form>
@endsection
@section('scripts')
    <script>
